...
document.addEventListener('DOMContentLoaded', () => {
    ...
    testModelsBtn.addEventListener('click', async () => {
        ...
        try {
            ...
            statusDiv.textContent = '❌ Model test failed. Check console for details.';
        }
    });
                ...
                chrome.tabs.query({ active: true, currentWindow: true }, async (tabs) => {
                    if (tabs[0] && tabs[0].url.includes("face-recognise.vercel.app")) {
                        ...
                        try {
                            ...
                            console.error('Direct autofill failed, trying storage method:', error);
                                    ...
                                    func: () => {
                                        ...
                                        chrome.storage.local.get('autofillData', (result) => {
                                            if (result.autofillData && window.fillForm) {
                                                setTimeout(() => window.fillForm(result.autofillData), 500);
                                                chrome.storage.local.remove('autofillData');
                                            }
                                        });
                                    }
                                });
                                statusDiv.textContent = 'Form auto-filled successfully (fallback method)!';
                            } catch (fallbackError) {
                                console.error('Fallback autofill also failed:', fallbackError);
                                statusDiv.textContent = 'Autofill failed. Check console for errors.';
                            }
                        }
                    } else {
                        // Save data and inform user to navigate to the site
                        await chrome.storage.local.set({ 'autofillData': emailData });
                        statusDiv.textContent = 'Data saved! Navigate to face-recognise.vercel.app/add-image and click Auto-fill again.';
                    }
                });
            }
        }
...
function autofillForm(emailData) {
    ...
    function simulateInput(element, value) {
        ...
        console.log(`Filled field with selector, old value: "${originalValue}", new value: "${value}"`);
    }
    ...
    setTimeout(async () => {
        try {
            ...
            if (emailField && emailData.senderEmail && emailData.senderEmail !== 'N/A') {
            ...
            if (uploadElement && emailData.images && emailData.images.length > 0) {
                console.log('Found upload area with', emailData.images.length, 'images available');
