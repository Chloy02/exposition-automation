...
class TrackingFaceDetector {
    ...
    async init() {
        try {
            ...
            console.error("❌ Failed to initialize tracking.js face detector:", error);
            this.ready = false;
            return false;
        }
    }
    ...
    async detectFaces(imageElement) {
        ...
        try {
            console.log(`Starting face detection on ${imageElement.width}x${imageElement.height} image...`);
            ...
            console.log(`✅ Face detection complete: found ${scaledFaces.length} face(s)`);
            scaledFaces.forEach((face, i) => {
                console.log(`  Face ${i + 1}: x=${face.x}, y=${face.y}, w=${face.width}, h=${face.height}`);
            });
    ...
    async cropFaces(imageElement, faces) {
        ...
        for (let i = 0; i < faces.length; i++) {
            ...
            let cropWidth = Math.min(imageElement.width - cropX, face.width + 2 * paddingX);
            let cropHeight = Math.min(imageElement.height - cropY, face.height + 2 * paddingY);
            ...
            console.log(`✅ Cropped face ${i + 1}/${faces.length}: ${cropWidth}x${cropHeight} at (${cropX}, ${cropY})`);
        }
